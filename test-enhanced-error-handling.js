...
async function testDecorators() {
  ...
  try {
    ...
    try {
      ...
      console.log('错误处理装饰器成功结果:', result1);
    } catch (error) {
      console.log('错误处理装饰器失败:', error.message);
    }
    
    try {
      const result2 = await service.riskyOperation(true);
      console.log('错误处理装饰器降级结果:', result2);
    } catch (error) {
      console.log('错误处理装饰器最终失败:', error.message);
    }
    
    // 测试日志装饰器
    const logResult = await service.loggedOperation('test_data');
    console.log('日志装饰器结果:', logResult);
    
    // 测试弹性装饰器
    try {
      const resilientResult1 = await service.resilientOperation(false);
      console.log('弹性装饰器成功结果:', resilientResult1);
    } catch (error) {
      console.log('弹性装饰器失败:', error.message);
    }
    
    try {
      const resilientResult2 = await service.resilientOperation(true);
      console.log('弹性装饰器降级结果:', resilientResult2);
    } catch (error) {
      console.log('弹性装饰器最终失败:', error.message);
    }
    ...
    for (let i = 0; i < 4; i++) {
      try {
        ...
        console.log(`断路器装饰器测试 ${i + 1}:`, circuitResult);
      } catch (error) {
        console.log(`断路器装饰器测试 ${i + 1}:`, error.message);
      }
    }
