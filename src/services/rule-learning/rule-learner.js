...
export class IntelligentRuleLearner {
  ...
  async learnFromAnalysis(analysisResult, sqlQuery) {
    try {
      if (!this.config.enabled) {
        console.log(`[RuleLearner] 规则学习已禁用`);
      ...
      if (!shouldLearn.should) {
        console.log(`[RuleLearner] 跳过学习: ${shouldLearn.reason}`);
  ...
  async batchLearnFromHistory(options = {}) {
    try {
      ...
      console.log(`[RuleLearner] 开始批量学习历史数据...`);
      ...
      console.log(`[RuleLearner] 批量学习完成: 处理${processedPatterns}个模式，生成${optimizedRules.length}条规则`);
  ...
  async performBatchLearning(options = {}) {
    try {
      console.log('[RuleLearner] 开始执行批量学习...');
  ...
  async deepLearningAnalysis(historyRecords) {
    try {
      console.log(`[RuleLearner] 开始深度学习分析...`);
      ...
      console.log(`[RuleLearner] 深度学习完成: 识别${learningResult.patternAnalysis?.length || 0}个模式，生成${learningResult.new_rules?.length || 0}条规则`);
  ...
  async saveToIssues(rules, context) {
    try {
      ...
      console.log(`[RuleLearner] 规则已保存到: ${filePath}`);
  ...
  async saveRulesByApproval(evaluatedRules, approvedRules, context) {
    try {
      console.log(`[RuleLearner] 开始分类保存 ${evaluatedRules.length} 条规则...`);
  ...
  async saveToApproved(rule, context, approvalInfo) {
    try {
      ...
      console.log(`[RuleLearner] 规则已审批: ${filePath}`);
  ...
  async saveToManualReview(rule, context, decision) {
    try {
      ...
      console.log(`[RuleLearner] 规则已移至人工审核: ${filePath}`);
