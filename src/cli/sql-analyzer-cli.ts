...
import type { AnalysisOptions, CLIOptions } from '../types/index.js';
...
class SQLAnalyzerCLI {
   ...
   * @param args 参数
   */
  async handleAnalyze(args: string[]): Promise<void> {
    if (args.length === 0) {
    ...
    try {
      ...
      if (result.success) {
        this.displayFileResults(result as FileAnalysisResult);
      ...
      console.error('❌ 文件分析失败:', (error as Error).message);
      process.exit(1);
    }
  }
   ...
   * @param args 参数
   */
  async handleDirectory(args: string[]): Promise<void> {
    if (args.length === 0) {
    ...
    try {
      ...
      if (result.success) {
        this.displayDirectoryResults(result as DirectoryAnalysisResult);
        const duration = Date.now() - startTime;
      ...
      console.error('❌ 目录分析失败:', (error as Error).message);
      process.exit(1);
    }
  }
   ...
   * @param args 参数
   */
  async handleStats(args: string[]): Promise<void> {
    ...
    console.log('');
    console.log('⚙️ 当前配置:');
    console.log(`LLM模型: ${config.get('llm.model') || 'unknown'}`);
   ...
   * @param result 分析结果
   */
  displayFileResults(result: FileAnalysisResult): void {
    console.log('');
    console.log('📄 文件分析结果');
   ...
   * @param result 分析结果
   */
  displayDirectoryResults(result: DirectoryAnalysisResult): void {
    console.log('');
    console.log('📁 目录分析结果');
    console.log('='.repeat(50));
    ...
    if (result.results && result.results.length > 0) {
      ...
      result.results.forEach((fileResult, index) => {
        if (fileResult.success) {
          ...
          console.log(`${index + 1}. ${(fileResult as any).fileName || 'Unknown'} - 分析失败`);
        }
      });
    }
  }
