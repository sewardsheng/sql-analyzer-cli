...
export class SmartPromptBuilder {
  ...
  private componentCache: Map<string, PromptResult>;
  private stats: Omit<BuilderStats, 'cacheSize' | 'cacheHitRate'>;
   ...
   * @returns 构建的提示词
   */
  async buildPrompt(context: AnalysisContext, buildOptions: BuildOptions = {}): Promise<PromptResult> {
    ...
    try {
      ...
      logError('构建提示词失败', error as Error);
      throw new Error(`提示词构建失败: ${(error as Error).message}`);
    }
  }
   ...
   * @returns 系统提示词
   */
  async buildSystemPrompt(context: AnalysisContext, options: BuildOptions = {}): Promise<string> {
   ...
   * @returns 用户提示词
   */
  async buildUserPrompt(context: AnalysisContext, options: BuildOptions = {}): Promise<string> {
    const components: string[] = [];
   ...
   * @returns 系统角色定义
   */
  private buildSystemRoleComponent(context: AnalysisContext): string {
   ...
   * @returns 任务定义
   */
  private buildTaskDefinitionComponent(context: AnalysisContext): string {
    const analysisTypes = context.core?.analysisTypes || [];
  ...
  /**
   * 构建数据库指导组件
   * @param database 数据库上下文
   * @returns 数据库指导
   */
  private buildDatabaseGuidanceComponent(database: DatabaseContext): string {
   ...
   * @returns 分析指导
   */
  private buildAnalysisGuidelinesComponent(context: AnalysisContext): string {
    const complexity = context.metadata?.complexity || 'medium';
   ...
   * @returns 约束条件
   */
  private buildConstraintsComponent(context: AnalysisContext): string {
    const analysisTypes = context.core?.analysisTypes || [];
    let constraints = '\n\n## 约束条件\n\n';
   ...
   * @returns 输出格式
   */
  private buildOutputFormatComponent(context: AnalysisContext): string {
    const analysisTypes = context.core?.analysisTypes || [];
  ...
  /**
   * 构建知识库指导组件
   * @param knowledge 知识库上下文
   * @returns 知识库指导
   */
  private buildKnowledgeGuidanceComponent(knowledge: KnowledgeContext): string {
    let guidance = '\n\n## 知识库参考\n\n';
   ...
   * @returns 历史参考
   */
  private buildHistoryReferenceComponent(history: any[]): string {
    let reference = '\n\n## 历史分析参考\n\n';
   ...
   * @returns 优化后的提示词
   */
  private optimizeSystemPrompt(components: string[], context: AnalysisContext): string {
    // 根据优先级和复杂度优化组件顺序
   ...
   * @returns 压缩后的提示词
   */
  private compressSystemPrompt(prompt: string, ratio: number): string {
    // 移除示例部分，保留核心指令
   ...
   * @returns 压缩后的提示词
   */
  private compressUserPrompt(prompt: string, ratio: number): string {
    // 压缩历史参考和详细选项
   ...
   * @returns 描述
   */
  private getAnalysisTypeDescription(type: AnalysisType): string {
   ...
   * @returns 重点描述
   */
  private getAnalysisFocusDescription(type: AnalysisType): string {
    const config = ANALYSIS_CONFIG[type];
    if (!config) return '';
   ...
   * @returns 格式化后的建议
   */
  private formatOptimizationTip(tip: string): string {
   ...
   * @returns 格式化后的模式
   */
  private formatAntiPattern(pattern: string): string {
   ...
   * @returns 缓存键
   */
  private generateCacheKey(context: AnalysisContext, options: BuildOptions): string {
    const keyData = {
 ...
 * @returns 构建器实例
 */
export function createSmartPromptBuilder(options: BuildOptions = {}): SmartPromptBuilder {
