 ...
 * @param app - Hono应用实例
 */
export function registerHistoryRoutes(app: Hono): void {
  /**
  ...
  app.get('/history/export', async (c: Context) => {
    try {
      ...
      if (format === 'csv') {
        ...
        return c.text(typeof result === 'string' ? result : JSON.stringify(result));
      } else {
   ...
   * }
   */
  app.post('/history/search', async (c: Context) => {
    try {
