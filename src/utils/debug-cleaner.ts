/**
* 调试日志清理工具
* 艹！老王要把这些SB的调试日志全都干掉！
*/

import fs from 'fs/promises';
import path from 'path';

/**
* 调试日志清理器
* 专门清理生产环境不需要的调试日志
*/
export class DebugCleaner {
  private options: {
    dryRun: boolean;
    backup: boolean;
    preserveComments: boolean;
  };

  private patterns: RegExp[];

  private preservePatterns: RegExp[];

  private stats: {
    filesProcessed: number;
    linesRemoved: number;
    errors: number;
  };

constructor(options = {}) {
this.options = {
dryRun: false,  // 是否只显示将要删除的行
backup: true,   // 是否创建备份
preserveComments: true, // 是否保留重要的注释
...options
};

// 需要清理的模式
this.patterns = [

/console\.log\s*\(\s*['"][^'"]*['"][^)]*\)/g,
/console\.log\s*\([^)]*DEBUG[^)]*\)/g,
/console\.log\s*\([^)]*debug[^)]*\)/g,

// console.warn 调试日志
/console\.warn\s*\(\s*['"][^'"]*加载.*失败[^'"]*['"][^)]*\)/g,

// 其他调试模式
/console\.(log|warn|info|debug)\s*\(\s*['"][🔧⚠️✅❌][^)]*\)/g,
/console\.(log|warn|info|debug)\s*\([^)]*加载.*路径[^)]*\)/g,
/console\.(log|warn|info|debug)\s*\([^)]*模板.*长度[^)]*\)/g
];

// 保留的重要日志模式
this.preservePatterns = [
/console\.log\s*\([^)]*服务器启动[^)]*\)/g,
/console\.log\s*\([^)]*API.*服务器[^)]*\)/g,
/console\.warn\s*\([^)]*警告[^)]*\)/g,
/console\.error\s*\([^)]*错误[^)]*\)/g
];

this.stats = {
filesProcessed: 0,
linesRemoved: 0,
errors: 0
};
}

/**
* 清理指定目录中的调试日志
* @param {string} dirPath - 目录路径
* @param {Array<string>} extensions - 文件扩展名
* @returns {Promise<Object>} 清理统计
*/
async cleanDirectory(dirPath, extensions = ['.js']) {
const files = await this.findJavaScriptFiles(dirPath, extensions);



for (const filePath of files) {
try {
await this.cleanFile(filePath);
this.stats.filesProcessed++;
} catch (error) {
console.error(`❌ 清理文件失败: ${filePath}`, error.message);
this.stats.errors++;
}
}







return this.stats;
}

/**
* 清理单个文件
* @param {string} filePath - 文件路径
* @returns {Promise<void>}
*/
async cleanFile(filePath) {
// 读取文件内容
const content = await fs.readFile(filePath, 'utf8');
const lines = content.split('\n');

// 创建备份
if (this.options.backup) {
const backupPath = filePath + '.backup.' + Date.now();
await fs.writeFile(backupPath, content);
}

// 清理每一行
const cleanedLines = [];
let linesRemoved = 0;

for (let i = 0; i < lines.length; i++) {
const line = lines[i];
const trimmedLine = line.trim();

// 检查是否需要保留
if (this.shouldPreserveLine(trimmedLine)) {
cleanedLines.push(line);
continue;
}

// 检查是否需要删除
if (this.shouldRemoveLine(trimmedLine)) {
linesRemoved++;
continue;
}

// 清理行内的小调试代码
const cleanedLine = this.cleanInlineDebug(trimmedLine);
cleanedLines.push(cleanedLine);
}

// 写入清理后的内容
const cleanedContent = cleanedLines.join('\n');

if (!this.options.dryRun && cleanedContent !== content) {
await fs.writeFile(filePath, cleanedContent);
}

if (linesRemoved > 0) {
console.log(`🗑️  ${path.relative(process.cwd(), filePath)}: 删除了 ${linesRemoved} 行调试日志`);
this.stats.linesRemoved += linesRemoved;
}
}

/**
* 查找JavaScript文件
* @param {string} dirPath - 目录路径
* @param {Array<string>} extensions - 文件扩展名
* @returns {Promise<Array<string>>} 文件路径数组
*/
async findJavaScriptFiles(dirPath, extensions) {
const files = [];

async function scanDirectory(currentPath) {
try {
const entries = await fs.readdir(currentPath, { withFileTypes: true });

for (const entry of entries) {
const fullPath = path.join(currentPath, entry.name);

if (entry.isDirectory()) {
// 跳过node_modules和.git目录
if (entry.name !== 'node_modules' && entry.name !== '.git') {
await scanDirectory(fullPath);
}
} else if (entry.isFile()) {
// 检查文件扩展名
const ext = path.extname(entry.name);
if (extensions.includes(ext)) {
files.push(fullPath);
}
}
}
} catch (error) {
console.error(`扫描目录失败: ${currentPath}`, error.message);
}
}

await scanDirectory(dirPath);
return files;
}

/**
* 判断是否应该保留行
* @param {string} line - 行内容
* @returns {boolean} 是否保留
*/
shouldPreserveLine(line) {
// 检查保留模式
for (const pattern of this.preservePatterns) {
if (pattern.test(line)) {
return true;
}
}

// 检查是否是重要注释
if (this.options.preserveComments &&
        (line.includes('@TODO') || line.includes('@FIXME') || line.includes('@IMPORTANT'))) {
return true;
}

return false;
}

/**
* 判断是否应该删除行
* @param {string} line - 行内容
* @returns {boolean} 是否删除
*/
shouldRemoveLine(line) {
// 检查删除模式
for (const pattern of this.patterns) {
if (pattern.test(line)) {
return true;
}
}

return false;
}

/**
* 清理行内调试代码
* @param {string} line - 行内容
* @returns {string} 清理后的行
*/
cleanInlineDebug(line) {
// 移除行内的小调试代码
return line
.replace(/console\.log\s*\(\s*['"][^'"]*['"]\s*\)\s*;?\s*$/g, '')
.replace(/console\.log\s*\([^)]*\)\s*;?\s*$/g, '')
.replace(/\/\/ console\.log.*/g, '')
.replace(/\/\* console\.log.*?\*\//g, '');
}

/**
* 获取清理统计
* @returns {Object} 统计信息
*/
getStats() {
return { ...this.stats };
}

/**
* 重置统计
*/
resetStats() {
this.stats = {
filesProcessed: 0,
linesRemoved: 0,
errors: 0
};
}
}

/**
* 清理调试日志的便捷函数
* @param {string} projectPath - 项目路径
* @param {Object} options - 清理选项
* @returns {Promise<Object>} 清理统计
*/
export async function cleanDebugLogs(projectPath = process.cwd(), options = {}) {
const cleaner = new DebugCleaner(options);
return await cleaner.cleanDirectory(projectPath);
}

/**
* 分析调试日志（预览模式）
* @param {string} projectPath - 项目路径
* @returns {Promise<Object>} 分析结果
*/
export async function analyzeDebugLogs(projectPath = process.cwd()) {
const cleaner = new DebugCleaner({ dryRun: true, backup: false });
const stats = await cleaner.cleanDirectory(projectPath);

return {
summary: `项目中发现 ${stats.linesRemoved} 行调试日志，分布在 ${stats.filesProcessed} 个文件中`,
details: stats,
recommendation: stats.linesRemoved > 0 ?
'建议运行清理命令移除调试日志' :
'项目代码已经很干净，无需清理'
};
}

export default DebugCleaner;