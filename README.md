...
### 技术特性
- **TypeScript支持**：完全用TypeScript重写，提供更好的类型安全和开发体验
- **多数据库支持**：MySQL、PostgreSQL、Oracle、SQL Server
- **批量分析**：支持批量SQL语句分析
- **历史记录**：完整的分析历史记录和管理
- **现代化架构**：基于Hono框架的高性能API服务器
## 📋 快速开始
...
本项目还提供了强大的命令行工具，支持直接在终端中进行SQL分析。

### 基本用法

```bash
# 查看帮助信息
pnpm run sql-analyzer --help

# 分析单个SQL文件
pnpm run analyze your_file.sql

# 查看分析器统计信息
pnpm run stats

# 扫描整个目录的SQL文件
pnpm run scan ./sql-directory
```
...
| 命令 | 等价于 | 功能 |
|------|--------|------|
| `pnpm run analyze <file>` | `tsx src/cli/sql-analyzer-cli.ts analyze <file>` | 分析单个SQL文件 |
| `pnpm run stats` | `tsx src/cli/sql-analyzer-cli.ts stats` | 显示统计信息 |
| `pnpm run scan <dir>` | `tsx src/cli/sql-analyzer-cli.ts directory <dir>` | 扫描目录 |

#### 全局命令安装
通过`package.json`的`bin`字段配置，可以将命令安装到全局：
...
# 发布后全局安装
npm install -g sql-analyzer-api
# 或
pnpm add -g sql-analyzer-api
```
...
# 分析文件
sql-analyzer analyze your_file.sql
...
# 使用别名
sql-analyzer a your_file.sql  # analyze的简写
sql-analyzer s                 # stats的简写
```
...
```bash
# 分析文件并显示详细信息
pnpm run analyze query.sql --database mysql --json

# 递归扫描目录
pnpm run sql-analyzer directory ./sql-files --recursive

# 仅执行性能分析
pnpm run sql-analyzer analyze query.sql --performance --no-cache
```
